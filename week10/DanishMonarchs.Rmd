---
title: "Danske konger homework week 10"
...
#loading the CSV-file into my Rmarkdown script

read_csv2("data/danskekonger.csv")
...
##loading my csv-file in diffrent ways to see if my csv-file is comma or semicolon-separated.

kings1 <- read.csv("data/danskekonger.csv")
glimpse(kings1)
kings2 <- read_csv("data/danskekonger.csv")
glimpse(kings2)
kings3 <- read.csv2("data/danskekonger.csv")
glimpse(kings3)
kings4 <- read_csv2( "data/danskekonger.csv")
glimpse(kings4)
```
Answer:

1. Which of these functions is a `tidyverse` function? Read data with it below into a `kings` object
   - The read_csv("data/kings.csv") and the read_csv2("data/kings.csv") are tidyverse functions. 
   
2. What is the result of running `class()` on the `kings` object created with a tidyverse function.
  - When I run the class() function on Kings, I get following anwser: "spec_tbl_df" "tbl_df"      "tbl"         "data.frame". This means that the data is a tibble, which means that it regonize numbers. 

3. How many columns does the object have when created with these different functions?
  - When I run kings1 and kings2 I get 56 rows and 1 column. This is because my CSV-file is semicolon-separated and not commaseparated and therefor needs a diffrents CSV-reader. When I run my kings3 and kings4, which regonizes semicolon-separated files, I get 56 rows and 6 columns. 
...
#naming my csv-file "Kings"
kings <- kings4


#Checking that my file is a tibble.
class(kings)
...
You can calculate the duration of reign in years with `mutate` function by subtracting the equivalents of your `indsaet.` from `afslut.` columns and writing the result to a new column called `duration`. But first you need to check a few things:
...
```{r}
# calling my csv-file for "danskekonger" to make my code look more smooth
danskekonger <- read_csv2( "data/danskekonger.csv")
na.omit(danskekonger)

#creating two new columns in my CSV-file using the mutate() function and the pipe-function; creating duration and midyear.

kings <- danskekonger %>% 
  mutate(duration = afslut.-indsaet.) %>% 
  mutate(midyear = indsaet. + duration/2) #skal bruges i "Challenge"
...
```{r}
#Using the mean() function to find the average reign periode for danish monarchs.  

mean(kings$duration, na.rm = TRUE)

# I put my mean() function into the summarize() in order to find the mean. Summarize() finds only one value. The result is the same. 
...
```{r}
#filtering kings who have ruled for more than 40 years to find the index numbers of the kings who have ruled the longest.  
#filter(kings, duration > 40)

#filtering the ruler who have ruled longer than the mean duration (19.72727) 
filter(kings, duration > 19.72727)


```

```{r}
#counting the number of kings who have ruled longer than the mean duration which is 19.72727 using the count() function.
count(filter(kings, duration > 19.72727))
...
#the numbers 41,55 and 51 are the index numbers on the kings who have ruled the longest. Maybe there is a smarter way, but I found it by using: filter(kings, duration > 40). This shoed the numbers of the longest ruling kings, and their indexnumbers. 

kings %>% 
  slice(41, 55, 51)

#Using mutate() function to calculate the number of days the monachs have ruled. 
kings %>% 
...
#mutate(midyear = indsaet. + duration/2)

# I created the midyear column in the same code chunk as I created the "duration" column.
# I use theme() to create funny looking visualizations 
